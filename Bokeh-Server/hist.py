...
def hist_tab(data):
    # Create histogram data based on selected airlines, delay range, and bin size
    def make_data(selected_names, start_range, end_range, bin_size):
        ...
        for i, airline in enumerate(selected_names):
            ...
            hist_df = pd.DataFrame({
                'proportion': hist / np.sum(hist),   # Normalize histogram
                'left': edges[:-1],                 # Left edges of bins
                'right': edges[1:]                  # Right edges of bins
            })
            # Add formatted columns for tooltip/info
            ...
            hist_df['color'] = Category20_16[i % len(Category20_16)]  # Assign a color

            df = pd.concat([df, hist_df], ignore_index=True)

        return ColumnDataSource(df)

    # Create a Bokeh figure with quad glyphs to show histogram
    ...
    checkbox = CheckboxGroup(labels=airline_names, active=[0, 1])  # Select airlines
    bin_slider = Slider(start=1, end=30, value=5, step=1, title="دانه‌بندی هیستوگرام (minutes)")  # Bin width
    range_slider = RangeSlider(start=-60, end=180, value=(-60, 120), step=5, title="بازه‌ی تاخیر")  # Delay range

    # Create initial data source and plot
    ...
    # Return the layout wrapped in a Bokeh TabPanel
